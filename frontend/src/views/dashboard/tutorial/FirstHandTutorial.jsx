...
import { SCENES, fraction, handSnapshot } from './firstHandStory';
...
export default function FirstHandTutorial({ active, onPlay }) {
  ...
  const elapsed = reducedMotion ? SCENES[scene].duration : state.elapsed;
  ...
  const decision = chosenDecision ?? Math.min(2, Math.floor(elapsed / 5000));
  ...
  useEffect(() => setChosenDecision(null), [scene, generation]);
  ...
  const act = action => { interacted.current = true; if (action.type === 'toggle') setChosenDecision(null); dispatch(action); };
  const keyDown = event => {
  ...
  return <section ref={root} className={`first-hand${reducedMotion ? ' first-hand--reduced' : ''}`} aria-label='Your first hand: animated tutorial' tabIndex={0} onKeyDown={keyDown}>
    <div className='first-hand__phone'>
      ...
      <div className={`first-hand__stage first-hand__stage--${scene}`} key={generation}>
        ...
        {scene === 4 && <div className='first-hand__decision'>
          ...
          <div className='first-hand__decision-tabs' role='group' aria-label='Explore a decision'>{['Hit', 'Stand', 'Double Down'].map((label, index) => <button type='button' aria-pressed={decision === index} className={decision === index ? 'is-selected' : ''} key={label} onClick={() => { act({ type: 'pause' }); setChosenDecision(index); }}>{label}</button>)}</div>
      ...
      <div className='first-hand__caption'><p>{firstHand.lines[elapsed > firstHand.duration / 2 ? 1 : 0]}</p><small>{scene === 4 ? 'Decision example only. Your main hand stays at 21.' : scene === 5 ? 'Everyone stands. Betting settles. Cards reveal.' : scene === 6 ? 'Illustrative hand: 80-chip pot, no rake.' : 'A guided hand. No chips at risk.'}</small></div>
      ...
      <nav className='first-hand__controls' aria-label='Tutorial controls'>
        ...
        <div className='first-hand__transport'><button type='button' disabled={scene === 0} onClick={() => act({ type: 'move', direction: -1 })}>Back</button><button type='button' aria-label={playing ? 'Pause tutorial' : 'Play tutorial'} onClick={() => act({ type: 'toggle' })}>{playing ? 'Pause' : 'Play'}</button><button type='button' disabled={scene === 6} onClick={() => act({ type: 'move', direction: 1 })}>Next</button></div>
        <div className='first-hand__secondary'><button type='button' onClick={() => act({ type: 'replay', playing: !reducedMotion })}>Replay</button><span>{scene + 1} of 7</span><button type='button' onClick={() => act({ type: 'skip' })}>Skip Tutorial</button></div>
      </nav>
    </div>
  </section>;
}
