...
export default function PlayablePractice() {
    ...
    return <section className="ucd-practice ucd-playable" aria-label="Playable practice table">
        ...
        <div className="ucd-practice-room" key={deal} style={{ '--practice-backdrop': `url(${backdrop})` }}>
            ...
            <div className="ucd-replica-seats" aria-label="Practice players">{game.players.slice(1).map((player, i) => <div className={`ucd-replica-seat ucd-replica-seat-${i}${ended ? ' is-revealed' : ''}${player.folded ? ' is-folded' : ''}`} key={player.name} style={{ '--deal-delay': `${i * 120}ms` }}>
                ...
                <span className="ucd-seat-feedback" key={player.status} aria-hidden="true" />
                {/^(Called|Bet)/.test(player.status) && !ended && <span key={player.chips} className="ucd-chip-move" aria-hidden="true">21</span>}
                {ended ? <><Cards cards={player.cards} label={`${player.name}'s private card`} /><strong>{player.folded ? 'Folded' : totalFor(player, game.board) > 21 ? 'Bust' : `Total ${totalFor(player, game.board)}`}</strong></> : <div className="ucd-cards" role="group" aria-label={`${player.name}'s private card`}><span className="ucd-card ucd-card-back" role="img" aria-label={`${player.name}'s card face down`}><span aria-hidden="true">♠</span><b aria-hidden="true">21</b><span aria-hidden="true">♠</span></span></div>}
            </div>)}</div>
            <div className="ucd-replica-table"><img className="ucd-replica-art" src={table} alt="Original 21 Hold’em game table" /><div className="ucd-replica-board"><div className="ucd-pot"><span>POT</span><strong className="ucd-pot-change" key={game.pot}>{game.pot} <small>chips</small></strong></div><h2>Community cards</h2><Cards cards={game.board} label="Practice community cards" /></div></div>
