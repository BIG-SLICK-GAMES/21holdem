...
export function textureCss(value) {
    ...
    return `body[data-site-theme] :is(.main-layout:not(.gameplay-layout), .main-layout:not(.gameplay-layout) .main-layout-background, .auth-layout, .common-layout) {background-image:${textureImage(t)} !important;background-size:${t.pattern === 'dimples' ? '18px 18px' : 'auto'} !important;}`;
}
