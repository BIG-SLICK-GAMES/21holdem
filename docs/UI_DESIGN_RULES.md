...
| Blue | `--ui-blue` and `--ui-blue-deep` for all secondary controls |
| Gold | Banner metallic gold `--ui-gold` for accents; `--ui-gold-gradient` for primary actions and selected controls |
| Borders | Shared blue/grey gradient borders on surfaces and controls, with blue/grey separators in connected rows |
...
| States | Blue/grey focus ring; muted disabled state; red for errors and green for success |
...
variants for readability. All lobby tabs use a black backdrop with animated gold bokeh. Controls retain the banner blue and gold palette.
